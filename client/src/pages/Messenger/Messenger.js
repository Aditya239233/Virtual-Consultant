...
const Messenger = () => {
  ...
  useEffect(()=>{axios.get('http://localhost:8000/retrieveAllConversationPartners',{
    params: {
      username:username
    },
        ...
        <div className="activeChat">
          <div className="activeChatWrapper">
           { previews.map(preview=>{
              return <MessagePreview name={preview.name} sender={preview.sender} timestamp={preview.timestamp} preview_text={preview.preview_text} username={username}/>
            })
          }
          </div>
        </div>
